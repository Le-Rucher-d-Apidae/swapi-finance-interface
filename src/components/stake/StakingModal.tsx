...
import { TokenAmount, Pair, ChainId, LIQUIDITY_TOKEN_SYMBOL, LIQUIDITY_TOKEN_NAME } from '@swapi-finance/sdk2'
...
export default function StakingModal({ isOpen, onDismiss, stakingInfo, userLiquidityUnstaked }: StakingModalProps) {
  ...
  const [autocompoundApproval, autocompoundApproveCallback] = useApproveCallback(
    parsedAmount,
  ...
  async function onStake() {
    ...
    if (stakingContract && parsedAmount && deadline) {
      if (
        (autocompound && autocompoundApproval === ApprovalState.APPROVED) ||
        (!autocompound && approval === ApprovalState.APPROVED)
      ) {
        const stakeFunction =
          autocompound && autocompoundContract ? autocompoundContract.deposit : stakingContract.stake
        await stakeFunction(`0x${parsedAmount.raw.toString(16)}`, { gasLimit: 350000 })
      } else if (signatureData) {
        const stakeWithPermitFunction =
          autocompound && autocompoundContract
  ...
  async function onAttemptToApprove() {
    ...
    if (nonce) {
      ...
      const spenderContractAddress =
        autocompound && autocompoundContract ? stakingInfo.autocompoundingAddress : stakingInfo.stakingRewardAddress
 ...
 */}
          ...
          <ProgressCircles
            steps={[
              (autocompound ? autocompoundApproval === ApprovalState.APPROVED : approval === ApprovalState.APPROVED) ||
                signatureData !== null
            ]}
            disabled={true}
          />
